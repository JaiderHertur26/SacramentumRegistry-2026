...
const ImportCiudadesForm = ({ isOpen, onClose }) => {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = async (event) => {
        try {
            ...
            // Manual Validation to separate Errors and Warnings
            // CORRECTION: Use dioceseId for validation context to match storage location
            ...
            json.data.forEach((item, index) => {
                ...
                if (isDuplicate) {
                    warnings.push({ index: idx, message: `Fila ${idx}: Ciudad duplicada "${nombre}" (será ignorada).` });
                } else {
                    validCount++;
                }
            });
  ...
  const handleConfirm = () => {
      ...
      // CORRECTION: Use dioceseId for storage
      const targetContextId = user?.dioceseId;
      if (!targetContextId) {
      ...
      if (filteredData.length > 0) {
          ...
          // Pass dioceseId as the context ID
          result = importCiudades(filteredJson, targetContextId, false);
      } else {
          // If no data to import (all duplicates), simulate success with 0 count
      ...
      if (result.success) {
           let msg = `${result.count} ciudades importadas a la Diócesis.`;
           if (duplicatesCount > 0) {
               msg = `${result.count} ciudades importadas, ${duplicatesCount} duplicadas ignoradas.`;
           }
  ...
  const columns = [
      ...
      { header: 'Contador', accessor: 'count' },
      { header: 'Peso', accessor: 'weight' },
  ];

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Importar Ciudades (Nivel Diocesano)">
        <div className="space-y-6 min-w-[600px]">
            <p className="text-gray-900 text-sm">
                Importe el catálogo de Ciudades para la Diócesis. Requiere estructura: data (nombre), source, count, weight.
            </p>
            {!preview && !validationResult?.errors?.length && (
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                    <input type="file" accept=".json" onChange={handleFileChange} className="hidden" id="city-upload" />
                    <label htmlFor="city-upload" className="cursor-pointer flex flex-col items-center gap-2">
