                            ...
                            <div className="shadow-2xl bg-white print:shadow-none min-h-[11in] w-[8.5in]">
                                <div ref={printComponentRef}>
                                    <ConfirmationPrintTemplate 
                                        libro={partida.book_number}
                                        folio={partida.page_number}
                                        numero={partida.entry_number}
                                        lugarConfirmacionDetalle={partida.lugarConfirmacionDetalle || partida.lugarConfirmacion}
                                        fechaConfirmacion={partida.sacramentDate}
                                        apellidos={partida.lastName}
                                        nombres={partida.firstName}
                                        fechaNacimiento={partida.birthDate}
                                        sex={partida.sex || partida.sexo} // Pass raw value to template
                                        nombrePadre={partida.fatherName}
                                        nombreMadre={partida.motherName}
                                        padrinos={partida.godparents}
                                        ministro={partida.minister}
                                        notaAlMargen={partida.notaMarginal || ''}
                                        parrocoNombre={auxiliaryData?.parroco || 'PÁRROCO ENCARGADO'}
                                        cargo={'Párroco'}
                                        parroquiaInfo={auxiliaryData || {}}
                                        // Pass full objects for marginal notes logic
                                        status={partida.status}
                                        annulmentDate={partida.annulmentDate}
                                        decreeDate={partida.decreeDate}
                                        annulmentDecree={partida.annulmentDecree}
                                        newBook={partida.newBook}
                                        newFolio={partida.newFolio}
                                        newNumber={partida.newNumber}
                                    />
                                </div>
                            </div>
                        </div>
