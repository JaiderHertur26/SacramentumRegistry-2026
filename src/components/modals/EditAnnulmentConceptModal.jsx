...
const EditAnnulmentConceptModal = ({ isOpen, onClose, concept, onSuccess }) => {
    ...
    const { user } = useAuth();
    const { updateConceptoAnulacion } = useAppData();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!parishId) throw new Error("No se identificó el contexto (parroquia o diócesis)");

            const result = await updateConceptoAnulacion(concept.id, formData, parishId);
            
            if (result.success) {
                toast({
                    title: "Éxito",
                    description: "Concepto de anulación actualizado correctamente",
            ...
            toast({
                title: "Error",
                description: error.message || "Error al actualizar el concepto",
                variant: "destructive"
            });
        } finally {
            setIsLoading(false);
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                ...
                >
                    <div className="flex items-center justify-between p-4 border-b border-gray-100">
                        <h2 className="text-lg font-bold text-gray-800">Editar Concepto de Anulación</h2>
