...
/**
 * Standardizes baptism record data for the entire application.
 * Ensures consistent key names and handles matrimonial notification data.
 */
export const normalizeBaptismPartida = (rawData) => {
    ...
    try {
        ...
        return {
            ...
            // Critical Fields for Notifications
        ...
        console.error("Critical error during normalization:", e);
        return getEmptyPartida();
    }
};

/**
 * Injects parish details into a normalized partida.
 */
...
/**
 * Validates if the baptism data is sufficient for printing or processing.
 */
export const validateBaptismPartidaStructure = (data) => {
    const missing = [];
    if (!data.nombres && !data.apellidos) missing.push('Identidad (Nombre/Apellido)');
    if (!data.fechaBautismo) missing.push('Fecha de Bautismo');
