...
import { generarNotaAlMargenCorrecta } from './notaAlMargenGenerator';
...
export const agregarNotaAlMargen = (partida, notificacionData, documentoId) => {
    ...
    try {
        ...
        // Generate the clean formatted note
        const notaFormateada = generarNotaAlMargenCorrecta(notificacionData);
        ...
        storedBaptisms[index] = {
            ...
            notaMarginal: nuevaNotaCombinada, // Keep for backwards compatibility
            notaAlMargen: nuevaNotaCombinada,
            notificacionMatrimonialId: documentoId,
            estadoNotificacion: 'afectada',
            notificacionData: notificacionData // Store the raw data for future reference if needed
        };
...
export const afectarPartidaBautismo = (partida, notificacionData, documentoId) => {
    console.log(`[Afectar Partida] Intentando afectar partida ID: ${partida?.id} en parroquia: ${partida?.parishId}`);
    const result = agregarNotaAlMargen(partida, notificacionData, documentoId);
    if (result.success) {
...
export const guardarNotificacionMatrimonial = (datos) => {
    const { partida, formData, parishId, createdBy } = datos;
    ...
    const baseNotificacionData = {
        ...
        parroquiaMatrimonio: formData.marriageParishName || formData.marriageParish, // Best effort
    ...
    if (partida.parishId === parishId) {
        ...
        const noteRes = afectarPartidaBautismo(partida, notificacionDataPrincipal, savedDocId);
        if (noteRes.success) {
