...
import ConfirmationSeatIndividualPage from './ConfirmationSeatIndividualPage';
import ConfirmationSeatBatchPage from './ConfirmationSeatBatchPage';

const ConfirmationSeatPage = () => {
  const { user } = useAuth();
  ...
  return (
    <DashboardLayout entityName={user?.parishName || "Parroquia"}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
           <h1 className="text-2xl font-bold text-[#2C3E50] font-serif bg-clip-text text-transparent bg-gradient-to-r from-[#2C3E50] to-[#4B7BA7]">
                Registro de Nuevas Confirmaciones
            </h1>
      ...
      <div className="min-h-[500px]">
        {mode === 'individual' ? <ConfirmationSeatIndividualPage /> : <ConfirmationSeatBatchPage />}
      </div>
    </DashboardLayout>
  );
};

export default ConfirmationSeatPage;
