...
const BaptismIndexPage = () => {
  const { user } = useAuth();
  const { getMisDatosList, getBaptisms } = useAppData();
  ...
  const [currentPrintFilter, setCurrentPrintFilter] = useState(null); // null = general, string = book number
  ...
  const loadData = () => {
    ...
    try {
      ...
      // Sort: Book -> Page -> Entry -> Date
      const sorted = [...allBaptisms].sort((a, b) => {
  ...
  const handlePrint = (bookNumber = null) => {
    ...
    // 2. Trigger print after a short delay to allow render
    setTimeout(() => {
         const printContent = document.getElementById('baptism-index-print');
         if (!printContent) return;
         
         const printWindow = window.open('', '', 'height=600,width=800');
         printWindow.document.write('<html><head><title>Índice de Bautismos</title>');
  ...
  return (
    <DashboardLayout entityName={user?.parishName || 'Parroquia'}>
      <Helmet>
        <title>Índice de Bautismos - Eclesia Digital</title>
        <meta name="description" content="Índice general de registros de bautismo de la parroquia." />
      </Helmet>

      <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#4B7BA7] font-serif">Índice de Bautismos</h1>
      ...
      <Modal isOpen={isBookModalOpen} onClose={() => setIsBookModalOpen(false)} title="Seleccionar Libro para Imprimir">
      ...
      <div id="baptism-index-print" className="hidden">
