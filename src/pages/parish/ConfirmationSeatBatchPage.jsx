...
const ConfirmationSeatBatchPage = () => {
    const { user } = useAuth();
    const { getPendingConfirmations, seatMultipleConfirmations } = useAppData();
    ...
    const handleBatchConfirm = async () => {
        ...
        const result = await seatMultipleConfirmations(selectedIds, user?.parishId);
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 border-b border-gray-200 font-semibold text-gray-600 uppercase tracking-wider text-xs">
                        <tr>
                            ...
                            <th className="px-6 py-4">Fecha Conf.</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {pendingConfirmations.map((conf, index) => (
                            <tr 
                                key={conf.id} 
                                className={`transition-colors cursor-pointer ${selectedIds.includes(conf.id) ? 'bg-blue-50/50' : 'hover:bg-gray-50'}`}
                                onClick={() => toggleSelection(conf.id)}
                            >
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="flex gap-6 text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-2 border-l border-gray-300 pl-6">
                            <span>Total: <strong>{pendingConfirmations.length}</strong></span>
                        </div>
                    </div>
...
export default ConfirmationSeatBatchPage;
