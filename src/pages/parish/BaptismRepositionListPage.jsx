...
const BaptismRepositionListPage = () => {
  ...
  const loadData = () => {
      ...
      setTimeout(() => {
        ...
        console.log("📥 [BaptismRepositionListPage] Decretos cargados:", data);
        setRecords(data || []);
  ...
  const handleDelete = (id) => {
      if (window.confirm("¿Está seguro de eliminar este decreto? Esta acción no se puede deshacer y no restaurará partidas asociadas automáticamente.")) {
  ...
  const columns = [
    ...
    { header: 'Titular', accessor: 'targetName', className: "font-medium" },
    ...
    { header: 'Estado', render: (row) => <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">{row.status === 'active' ? 'Activo' : row.status}</span>}
  ];
