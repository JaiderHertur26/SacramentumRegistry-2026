...
import { Save, X, Loader2, Search, Trash2, FileText, UserPlus, Heart } from 'lucide-react';
...
const EditDecreeReplacementSheetPage = () => {
    ...
    useEffect(() => {
        if (user?.parishId) {
            ...
            // Load Concepts (Filter by 'porReposicion')
    ...
    const loadDecrees = () => {
        const allDecrees = getDecreeReplacementsBySacrament(activeTab, user?.parishId);
    ...
    useEffect(() => {
        if (selectedDecreeId) {
            ...
            console.log("📥 [EditDecreeSheet] Cargando Decreto:", decree);
            ...
            if (decree) {
                setDecreeData(prev => ({
                    ...
                    conceptoAnulacionId: decree.conceptoAnulacionId || decree.conceptoId || ''
                }));

                // --- POPULATE "Datos de Nueva Partida" (Section 2) ---
                if (decree.datosNuevaPartida) {
                    const bd = decree.datosNuevaPartida;
                    console.log("📥 [EditDecreeSheet] Cargando datosNuevaPartida:", bd);
                    setNewPartida({
                        sacramentDate: bd.sacramentDate || bd.fecbau || bd.fecha_sacramento || '',
                        firstName: bd.firstName || bd.nombres || '',
                        lastName: bd.lastName || bd.apellidos || '',
                        birthDate: bd.birthDate || bd.fecnac || bd.fecha_nacimiento || '',
                        lugarNacimientoDetalle: bd.lugarNacimientoDetalle || bd.lugarn || bd.lugar_nacimiento || '',
                        fatherName: bd.fatherName || bd.padre || bd.padre_nombre || '',
                        motherName: bd.motherName || bd.madre || bd.madre_nombre || '',
                        tipoUnionPadres: bd.tipoUnionPadres || bd.tipohijo || bd.tipo_union || '1',
                        sex: bd.sex || bd.sexo || '1',
                        paternalGrandparents: bd.paternalGrandparents || bd.abuepat || bd.abuelos_paternos || '',
                        maternalGrandparents: bd.maternalGrandparents || bd.abuemat || bd.abuelos_maternos || '',
                        godparents: bd.godparents || bd.padrinos || '',
                        minister: bd.minister || bd.ministro || '',
                        ministerFaith: bd.ministerFaith || bd.dafe || bd.da_fe || '',
                        
                        // Mapping Civil Registry fields correctly
                        serialRegCivil: bd.serialRegCivil || bd.regciv || '',
                        nuipNuit: bd.nuipNuit || bd.nuip || '',
                        oficinaRegistro: bd.oficinaRegistro || bd.notaria || '',
                        fechaExpedicion: bd.fechaExpedicion || bd.fecregis || ''
                    });
                } else if (decree.newPartidaSummary) {
                    setNewPartida(prev => ({ ...prev, ...decree.newPartidaSummary }));
                }
            }
        }
    ...
    const handleSave = async (e) => {
        ...
        try {
            // Include civil registry mapping back to standard format
            const finalPartidaData = {
                ...newPartida,
            ...
            const updatedData = {
                ...
                datosNuevaPartida: finalPartidaData
            };

            console.log("📤 [EditDecreeSheet] Guardando cambios de Decreto:", updatedData);

            const result = updateDecreeReplacement(selectedDecreeId, updatedData, user?.parishId);

            if (result.success) {
                // IMPORTANT: Update actual registry record
                const decree = decrees.find(d => d.id === selectedDecreeId);
                let registryKey = '';
                
                if (activeTab === 'bautismo') registryKey = `baptisms_${user.parishId}`;
                if (activeTab === 'confirmacion') registryKey = `confirmations_${user.parishId}`;
                if (activeTab === 'matrimonio') registryKey = `matrimonios_${user.parishId}`;

                const allRecords = JSON.parse(localStorage.getItem(registryKey) || '[]');
                
                if (decree && decree.newBaptismIdRepo) {
                    const idx = allRecords.findIndex(b => b.id === decree.newBaptismIdRepo);
                    
                    if (idx !== -1) {
                        allRecords[idx] = {
                            ...allRecords[idx],
                            ...finalPartidaData, // Apply edits including civil registry mapped names
                ...
                // Refresh local list
                loadDecrees();
                toast({ title: "Guardado", description: "Decreto y partida supletoria actualizados.", className: "bg-green-50 border-green-200 text-green-900" });
            } else {
    ...
    const filteredDecrees = decrees.filter(d => {
        ...
        const decreeNum = `${d.decreeNumber || d.numeroDecreto || ''}`.toLowerCase();
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full h-full flex flex-col">
                    ...
                    <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 overflow-hidden min-h-0">
                                    ...
                                    filteredDecrees.map((decree) => (
                                        ...
                                        >
                                            <div className="font-bold text-gray-800 flex justify-between">
                                                <span>{decree.decreeNumber || decree.numeroDecreto}</span>
                                                <span className="text-[10px] font-normal text-gray-400">{decree.decreeDate || decree.fechaDecreto}</span>
                                            </div>
                                            <div className="text-gray-600 truncate text-xs mt-1 font-medium">{decree.targetName}</div>
                                            {decree.newPartidaSummary && (
                                                <div className="text-[10px] text-gray-400 mt-1">
                                                    L:{decree.newPartidaSummary.book || decree.newPartidaSummary.book_number} 
                                                    F:{decree.newPartidaSummary.page || decree.newPartidaSummary.page_number} 
                                                    N:{decree.newPartidaSummary.entry || decree.newPartidaSummary.entry_number}
                                                </div>
                                            )}
                                        </button>
                                    ))
                        ...
                        <div className="lg:col-span-3 h-full overflow-y-auto custom-scrollbar px-2">
                            <TabsContent value="bautismo" className="mt-0 pb-10">
                                {renderBautizosForm()}
                            </TabsContent>
                             <TabsContent value="confirmacion" className="mt-0 h-full flex items-center justify-center">
                            ...
                            <TabsContent value="matrimonio" className="mt-0 h-full flex items-center justify-center">
                                <div className="text-center text-gray-400">
                                    <FileText className="w-12 h-12 mx-auto mb-3 opacity-30" />
            ...
            <form onSubmit={handleSave} className="space-y-8 max-w-4xl mx-auto animate-in fade-in duration-300">
                ...
                <div className="bg-white rounded-lg shadow-sm border-l-4 border-blue-600 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-600 uppercase mb-1">Titular de Partida</label>
                            <Input 
                                name="targetName"
                         ...
                         {/* CONCEPTO DE REPOSICION SELECTOR */}
                ...
                <div className="bg-white rounded-lg shadow-sm border-l-4 border-green-600 p-6">
                    <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2 border-b pb-2">
                        <UserPlus className="w-5 h-5 text-green-600" /> SECCIÓN 2: DATOS DE NUEVA PARTIDA (SUPLETORIO)
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {/* TASK 7: Update Edit form rendering for civil registry fields */}
                        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                            ...
                            <div>
                                ...
                                <Input 
                                    name="serialRegCivil" 
                                    value={newPartida.serialRegCivil} 
                                    onChange={handleNewPartidaChange} 
                                    className={!newPartida.serialRegCivil ? "border-amber-300 bg-amber-50" : ""}
                                />
                            </div>
                            <div>
                                ...
                                <Input 
                                    name="nuipNuit" 
                                    value={newPartida.nuipNuit} 
                                    onChange={handleNewPartidaChange} 
                                    className={!newPartida.nuipNuit ? "border-amber-300 bg-amber-50" : ""}
                                />
                            </div>
                            <div>
                                ...
                                <Input 
                                    ...
                                    value={newPartida.oficinaRegistro} 
                                    onChange={handleNewPartidaChange} 
                                    className={!newPartida.oficinaRegistro ? "border-amber-300 bg-amber-50" : ""}
                                />
                            </div>
                            <div>
                                ...
                                <Input 
                                    ...
                                    value={newPartida.fechaExpedicion} 
                                    onChange={handleNewPartidaChange} 
                                    className={!newPartida.fechaExpedicion ? "border-amber-300 bg-amber-50" : ""}
                                />
                            </div>
                        </div>
                ...
                <div className="flex justify-between gap-4 pt-4 border-t border-gray-200 sticky bottom-0 bg-white p-4 shadow-lg rounded-t-lg z-10">
                    ...
                    <div className="flex gap-4">
                        <Button type="button" variant="outline" onClick={() => navigate('/parish/decree-replacement/view')} disabled={isSubmitting}>
                            Cancelar
                        </Button>
